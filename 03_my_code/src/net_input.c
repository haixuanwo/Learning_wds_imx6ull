 ...
 * @Date: 2025-12-30 16:41:05
 * @LastEditors: Clark
 * @LastEditTime: 2025-12-30 17:31:21
...
static int NetinputGetInputEvent(PInputEvent ptInputEvent)
{
	...
	{
		aRecvBuf[iRecvLen] = '\0';
		// printf("T --- Get Msg From %s : %s\n", inet_ntoa(tSocketClientAddr.sin_addr), ucRecvBuf);
		ptInputEvent->type 	= INPUT_TYPE_NET;
		// gettimeofday(&ptInputEvent->tTime, NULL);

        ptInputEvent->data = malloc(iRecvLen+1);
        memcpy(ptInputEvent->data, aRecvBuf, iRecvLen+1);
		return 0;
	}
	else
		return -1;
